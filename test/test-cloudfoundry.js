	...
	frameworks.forEach(framework => {
		buildTypes.forEach(buildType => {
			createTypes.forEach(createType => {
				describe('cloud-enablement:cloudfoundry with Java-' + framework + ' with buildType ' + buildType + ' and createType ' + createType, () => {
					beforeEach(() => {
						let bluemixJson = framework === 'spring' ? scaffolderSampleSpring : scaffolderSampleJava;
						return helpers.run(path.join(__dirname, '../generators/app'))
							.inDir(path.join(__dirname, './tmp'))
							.withOptions({bluemix: JSON.stringify(bluemixJson), frameworkType : framework, buildType : buildType, createType: createType});
					});

					it('manifest.yml is generated with correct content', () => {
						assert.file('manifest.yml');
						let manifestyml = yml.safeLoad(fs.readFileSync('manifest.yml', 'utf8'));
						if(framework === 'liberty' && buildType === 'maven') {
						...
						if(framework === 'spring' && buildType === 'gradle') {
					...
					it('pipeline.yml file is generated with correct content', () => {
						...
						stages.forEach(stage => {
							if(stage.name === 'Build Stage') {
								...
								assert(stage.jobs[1].script.includes('#!/bin/bash\nif  [[ -f ../post_build.sh ]]; then chmod +x ../post_build.sh; echo "executing the post_build script"; sh ../post_build.sh; else echo "the post_build script does not exist";fi'), 'Expected pipelineyml.stages[0].jobs[1].script to include "' + '#!/bin/bash\nif  [[ -f ../post_build.sh ]]; then chmod +x ../post_build.sh; echo "executing the post_build script"; sh ../post_build.sh; else echo "the post_build script does not exist";fi' + '", found : ' + stage.jobs[1].script);
							}
