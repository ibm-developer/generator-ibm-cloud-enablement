				...
				yml.safeLoadAll(fs.readFileSync('manifests/kube.deploy.yml', 'utf8'), data => {
					switch(i) {
						...
						case 1:
							...
							if(language === 'JAVA') {
								assertYmlContent(data.spec.template.spec.containers[0].readinessProbe.httpGet.path, '/' + applicationName.toLowerCase() + '/health', 'doc1.spec.template.spec.containers[0].readinessProbe.httpGet.path');
							}
							if(language === 'SPRING') {
